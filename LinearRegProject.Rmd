...
### Section 2.2: Each loan term default rate v.s Term
```{r}
SBA_NY_Term <- SBA_NY %>%
  mutate(LoanTerm = case_when(Term >= 120 ~ "Long",
                              Term < 120 ~ "Short")) %>%
  group_by(LoanTerm) %>%
  summarize(CHGOFF = sum(MIS_Status =="CHGOFF"),
            PIF = sum(MIS_Status =="P I F"), 
...
### Section 2.3: Each company size default rate v.s Term
```{r}
SBA_NY_Size <- SBA_NY %>%
  mutate(Size = case_when(NoEmp <= 100 ~ "Small",
                          NoEmp > 100 & NoEmp <=999 ~ "Median",
                          NoEmp >999 ~ "Large" )) %>%
  group_by(Size) %>%
  summarize(CHGOFF = sum(MIS_Status =="CHGOFF"),
            PIF = sum(MIS_Status =="P I F"), 
